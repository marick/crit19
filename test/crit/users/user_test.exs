defmodule Crit.Users.UserTest do
  ...
  test "trying to reuse an auth id" do
    ...
    assert {:ok, _} = Users.create_unactivated_user2(first_params, @default_institution)

    second_params = Factory.string_params_for(:user, auth_id: first_params["auth_id"])
    assert {:error, changeset} = Users.create_unactivated_user2(second_params, @default_institution)
